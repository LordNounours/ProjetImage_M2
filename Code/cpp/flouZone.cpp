...
void flou(unsigned char *ImgIn, unsigned char *ImgOut, int nH, int nW, int k, float sigma, int x_start, int y_start, int x_end, int y_end) {
...
int main(int argc, char* argv[]) {
    ...
    int nH, nW, nTaille, k;
    float sigma;
    int x_start, y_start, x_end, y_end; //zone de flou, ce sera un rectangle

    if (argc != 9) {
        printf("Usage: ImageIn.png ImgOut.png k sigma x_start y_start x_end y_end\n");
        exit(1);
    }
    ...
    flou(ImgIn, ImgOut, nH, nW, k, sigma, x_start, y_start, x_end, y_end);
